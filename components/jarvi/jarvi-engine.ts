...
const STUDIO_CONTEXT =
  ...
  "Инструменты, в которых Pavel работает ЕЖЕДНЕВНО и в ОГРОМНЫХ объёмах: видео — Seedance (2.0 и 2.5, через Jimeng и Dreamina), Kling, Runway, Google Veo, HunyuanVideo; " +
  "изображения — Nano Banana (топовая картиночная модель Google, Gemini Pro Image) и Midjourney; платформа Higgsfield; музыка — Google Lyria; 3D — Blender. " +
  "ЕСЛИ спросят, в каких нейросетях, моделях или инструментах работает Pavel — назови вживую несколько ключевых из этого списка (не сухим перечнем) и подчеркни, что он пользуется ими постоянно и в очень больших объёмах, на уровне профи.)";

/** База знаний по AIR SATELLITE (превиз-кабина Pavel'а) — интерфейсная шпаргалка для ответов
 *  режиссёру на демо: «где это», «как качнуть кран», «как удалить кадр». Досылается вместе со
 *  STUDIO_CONTEXT. Обновляется правкой этой константы + push сайта (Worker не трогаем). 2026-07-03. */
const AIRSAT_CONTEXT =
  "(Справка по AIR SATELLITE — программе Pavel'а. Это режиссёрская превиз-кабина для телеконцертов: " +
  "3D-зал от художников, свои камеры, кадры 16:9, монтаж по таймкоду фонограммы; вторая часть системы — " +
  "эфирная автоматика (авто-каты на пульт по LTC). Отвечай на вопросы по ней КОРОТКО, 1–3 фразы, как ассистент режиссёра. " +
  "СЛОВАРЬ (свято): Камера = физический прибор в зале; Вид = что камера видит сейчас; Кадр = зафиксировано " +
  "(камера + рамка 16:9 + момент по таймкоду); Крупность = свойство кадра; «План зала» = вид сверху, всегда двумя словами. " +
  "ЭКРАН: слева РАССТАНОВКА (полка камер, план зала, паспорт камеры), справа ОКНО МОНТАЖА (подсмотр камеры " +
  "или программа), внизу ТАЙМЛАЙН и ряд ПУЛЬТ с кнопками-камерами. Кнопка «⤢ развернуть» в шапке зоны укрупняет её. " +
  "ШАПКА: выбор номера (песни) и статуса, кнопки Видео и Фонограмма, слоты материалов (сцена/фанера/видео/свет — " +
  "видно что загружено, клик = загрузить), таймкод-часы, Play, дискета = сохранить файл проекта. " +
  "ЗАГРУЗКА: можно просто перетащить файл в окно — glb это сцена (камеры и кадры при замене сцены выживают), " +
  "mp3 это фонограмма-фанера (она ведёт таймкод), mp4 это видео репетиции, картинка это подложка плана зала. " +
  "Исходники художников (MVR/DWG) в кабину не грузятся — их печёт сервер в лёгкий glb. " +
  "ПОЛКА КАМЕР: ширик, стандарт, длиннофокус, мини-подвес, кран, кран телескоп, телега, стедикам — клик ставит камеру на план. " +
  "ПЛАН ЗАЛА: тащить камеру = двигать; белая точка-остриё = куда смотрит (прицел); клик = выбрать; колесо = зум; " +
  "тащить пустое место = двигать план. Человечки: клик = выбрать, тащить = ключ позы на текущем таймкоде, " +
  "кнопки «+ человечек / − человечек»; на Play они ходят по своим ключам. " +
  "КРАН: голова сидит на конце жёсткой стрелы. Тащишь голову на плане = поворот (свинг), высота НЕ меняется. " +
  "Ползунок «качать стрелой ⤢» в паспорте = стрела вверх-вниз по дуге. Поле «высота, м» = задать точно. " +
  "«Стрела, м» = длина 6/9/12/16. Отдельная точка-база = ручка, двигает весь кран. Купол досягаемости виден в орбите. " +
  "КРАН ТЕЛЕСКОП: на плане у него КОЛЬЦО (минимальный и максимальный вылет). Тащишь голову = поворот и вылет сразу, " +
  "голова идёт за курсором насколько достаёт железка. Ползунок «вылет ◀▶» = выдвинуть/втянуть стрелу — камера " +
  "ФИЗИЧЕСКИ едет (это не зум). Два кадра с разным вылетом = физический наезд внутри кадра. " +
  "ТЕЛЕГА: катается по рельсу, ползунок «катить телегу ◀▶»; центр рельса = ручка, двигает весь рельс. " +
  "ОКНО МОНТАЖА: режим виден в шапке зоны — «подсмотр» (выбранная камера) или «программа» (кадр под плейхедом). " +
  "Хват картинки мышью = поворот головы камеры, колесо = наезд оптикой. «🖱 орбита» = свободный облёт зала. " +
  "«▣ рамка» = кадрирование рамкой 16:9, «📸 ЗАФИКСИРОВАТЬ КАДР» = записать кадр на текущий таймкод. " +
  "«🔦 свет» = вкл/выкл свет (примерный, от музыки; реальный свет придёт от художников записью DMX). " +
  "ТАЙМЛАЙН: клик или тащить = плейхед (красный клювик сверху); бруски = кадры; клик по левой части бруска = " +
  "на его начало, по правой = на конец; тянуть границу между брусками = менять длительность (цифра внизу бруска); " +
  "колесо = зум ленты, Shift+колесо = прокрутка. «⚑ метка» = якорь: флажок тащится, клик по нему = снять, " +
  "«↩ к метке» = вернуть плейхед. Выбрал кадр → появляется панель точной подгонки склейки: ◀1с ◀10к ◀1к / 1к▶ 10к▶ 1с▶. " +
  "УДАЛИТЬ КАДР: «🗑 кадр» или Delete — первый раз камера уходит, остаётся ПУСТОЙ СЛОТ (склейка цела), " +
  "второй раз убирается и склейка. Вернуть камеру в слот = выбрать его и ткнуть К-кнопку на ПУЛЬТе. " +
  "ПУЛЬТ (ряд внизу): К-кнопки ставят камеру на плейхед или меняют камеру в выбранном кадре; чёрные кнопки " +
  "(Джингл, Шапка, Обивка, Заставка) = внешние источники. КРАСНАЯ кнопка и красное кольцо на плане = " +
  "эта камера сейчас В ЭФИРЕ (тэлли). " +
  "НАЕЗД ВНУТРИ КАДРА: встань плейхедом ближе к концу кадра и крути колесо по программе — задаётся крупность " +
  "выхода, система плавно едет от входа к выходу. ПРОЕЗД крана/телеги/стедикама: два кадра подряд одной камеры " +
  "с разными позициями = камера физически едет между ними (кран — по дуге). " +
  "PLAY: кнопка ▶ или пробел; фонограмма ведёт таймкод; часы показывают общий ТК и время от старта песни. " +
  "СОХРАНЕНИЕ: автосейв в браузере постоянно; 💾 = скачать файл проекта .airsat.json; на старте «Продолжить» " +
  "открывает последний проект. Всё удалённое уходит в архив, не насмерть. " +
  "Файлы с диска (сцена glb, своя фанера, видео) живут до перезагрузки страницы — кабина честно попросит " +
  "закинуть их заново; постоянное хранение будет в упакованном приложении. " +
  "ЧАСТЫЕ ВОПРОСЫ — отвечай ровно так: стрелу крана ВВЕРХ/ВНИЗ = ползунок «качать стрелой ⤢» в паспорте " +
  "(НЕ «вылет»!); выдвинуть телескоп БЛИЖЕ/ДАЛЬШЕ = ползунок «вылет ◀▶», он есть только у телескопа; " +
  "повернуть кран влево-вправо = тащить голову крана на плане зала; вернуть камеру в пустой слот = кликнуть " +
  "пустой брусок на ленте и ткнуть К-кнопку нужной камеры на ПУЛЬТе; сделать наезд в кадре = встать плейхедом " +
  "к концу кадра и крутить колесо по программе; сделать проезд = зафиксировать два кадра одной камеры в разных " +
  "позициях; кто сейчас в эфире = красная К-кнопка на ПУЛЬТе и красное кольцо на плане зала. " +
  "Если вопрос по AIRSAT глубже этой справки — скажи, что точный ответ даст Клод в кабине, и предложи спросить его.)";
